...
func _ready():
	...
	var otstup=10
	$charters/hrs.columns=int($charters/hrs.rect_size.x/sizex)
	for e in list:
		var btn=Button.new()
		btn.name=str(e)
		btn.expand_icon=true
		btn.rect_min_size=Vector2(sizex,sizex)
...
func refresh_lobby():
	...
	#$Players/Start.disabled = not (get_tree().is_network_server() and can_start_game==true and allplayers<c_builders*2+c_other_heroes*2)
...
func _on_op_item_selected(index):
	$charters/txt.text=$charters/op.text
